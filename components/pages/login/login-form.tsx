...
export function LoginForm({
  ...
  const handleLogin = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (hasAdminRole) {
        ...
        toast.success("Chào mừng trở lại, " + (userData.fullName || "Admin") + "!");
        router.push("/");
      } else {
        toast.error("Truy cập bị từ chối. Cần có quyền Quản trị viên.");
        authStore.clearAuth();
      }
    } catch (error: any) {
  ...
  const handleGoogleLogin = async () => {
    ...
    if (typeof window === 'undefined' || !(window as any).google?.accounts?.id) {
      toast.error('Thư viện Google Sign-In chưa được tải. Vui lòng tải lại trang.');
      ...
      callback: async (response: any) => {
        try {
          ...
          if (!apiResponse.success) {
            throw new Error(apiResponse.message || "Đăng nhập Google thất bại");
          }
          ...
          <div className="space-y-4">
            <FormField
              ...
              render={({ field }: { field: ControllerRenderProps<LoginFormData, "email"> }) => (
                <FormItem>
                  <FormLabel className="text-xs font-semibold uppercase tracking-wider text-muted-foreground ml-1">Địa chỉ Email</FormLabel>
                  <FormControl>
              ...
              render={({ field }: { field: ControllerRenderProps<LoginFormData, "password"> }) => (
                <FormItem>
                  <div className="flex items-center justify-between ml-1">
                    <FormLabel className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Mật khẩu</FormLabel>
                    <Link
      ...
      <div className="space-y-4">
        ...
        <div className="relative py-2">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-background px-2 text-muted-foreground font-medium">Hoặc đăng nhập bằng Email</span>
          </div>
        </div>
      </div>
